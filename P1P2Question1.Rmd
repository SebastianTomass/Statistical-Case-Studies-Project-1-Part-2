...
ggplot(bac, aes(x = age, y = beta60)) +
  ...
  geom_smooth(method = "lm", se = FALSE, linetype = 2) +              # linear trend
  stat_quantile(quantiles = 0.025, method = "rq", formula = y ~ x,    # τ = 0.025 tail
                linewidth = 1, color = "red") +
  geom_rug(alpha = 0.2) +
  labs(x = "Age (years)",
       y = expression(beta~"(g/kg/h)"),
       title = "Age vs elimination rate β",
       subtitle = "Dots = observations; linear (dashed), 2.5th-quantile line (red)") +
  theme_minimal()
