...
```{r}
probs95   <- c(0.025, 0.975)
probs9973 <- c(0.00135, 0.99865)

bounds_emp <- bac %>%
  filter(!is.na(beta), sex %in% c("female","male")) %>%
  group_by(sex) %>%
  summarise(
    low95    = as.numeric(quantile(beta60, probs95[1],  na.rm = TRUE, names = FALSE)),
    high95   = as.numeric(quantile(beta60, probs95[2],  na.rm = TRUE, names = FALSE)),
    low9973  = as.numeric(quantile(beta60, probs9973[1], na.rm = TRUE, names = FALSE)),
    high9973 = as.numeric(quantile(beta60, probs9973[2], na.rm = TRUE, names = FALSE)),
    n = n(),
    .groups = "drop"
  )
...
Overall, Police Scotland's model is too conservative for males and too liberal for females, causing a systematic disadvantage for females. We recommend replacing the current protocol with calculating $C_o=C_t+\beta t$ using the 99.73% confidence intervals separate for men and females. EXPLAIN WHY